...
% spatial based-------------
There is another kind of convolutional approach for graphs, called spatial-based graph convolutional networks, see \cite{graphsage} and \cite{geometricdl}. They define graph convolution based on a node's spatial relations. These methods directly perform the convolution in the graph domain by aggregating the neighbor nodes  information. The spatial-based graph convolution takes the aggregation of the central node representation and its neighbors representation to get a new representation for this node. A common practice is to stack multiple graph convolution layers together.
...
The remaining part of the subsection will present evolutions of the GCN model that most commonly known.
...
% 		- Microsoft succesfull paper using GGNN
